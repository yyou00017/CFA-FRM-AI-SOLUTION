...
function withTimeout<T>(promise: Promise<T>, timeoutMs: number) {
  return Promise.race([
    ...
    new Promise<T>((_, reject) => {
      setTimeout(() => reject(new Error(`Question preparation timed out after ${timeoutMs}ms`)), timeoutMs);
    }),
  ]);
}
...
export default async function handler(req: any, res: any) {
  ...
  const fallback = (attempts = 1) => {
    ...
    return sendControlled(
      controlled,
      "local-backup",
      "Prepared by the reserve question bank because the primary service was unavailable or did not pass quality checks."
    );
  };
...
- Return only JSON that matches the schema.`,
      ...
      if (!shouldRetryForQuality(controlled, count)) {
        return sendControlled(
          controlled,
          "gemini-api",
          "Returned without a second service retry; the quality check accepted the strongest questions."
        );
      }
    }
